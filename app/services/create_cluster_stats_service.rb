...
class CreateClusterStatsService
  ...
  def call
    ...
    if last_epochs.count > 0
      epochs_duration = last_epochs.second.created_at - last_epochs.first.created_at
      ...
      epochs_annual_roi = (365 * epochs_duration * epochs_average_roi.count) / epochs_average_roi.sum.to_f
    end
    epochs_annual_roi ||= 0
