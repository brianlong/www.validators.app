...
module DataCenters
  class ChangeValidatorDataCenter
    ...
    def initialize(validator_id)
      ...
      @maxmind_results = nil
      @data_center_from_maxmind_data = nil
      @update_score = false
    ...
    def max_mind_results_for_ip
      ...
      @maxmind_results = client.insights(@validator_ip.address)
    end

    def data_center_from_maxmind_data
      @data_center = DataCenter.find_or_initialize_by(
        continent_code: @maxmind_results.continent.code,
        continent_geoname_id: @maxmind_results.continent.geoname_id,
        continent_name: @maxmind_results.continent.name,
        country_iso_code: @maxmind_results.country.iso_code,
        country_geoname_id: @maxmind_results.country.geoname_id,
        country_name: @maxmind_results.country.name,
        registered_country_iso_code: @maxmind_results.registered_country.iso_code,
        registered_country_geoname_id: @maxmind_results.registered_country.geoname_id,
        registered_country_name: @maxmind_results.registered_country.name,
        traits_anonymous: @maxmind_results.traits.anonymous?,
        traits_hosting_provider: @maxmind_results.traits.hosting_provider?,
        traits_user_type: @maxmind_results.traits.user_type,
        traits_autonomous_system_number: @maxmind_results.traits.autonomous_system_number,
        traits_autonomous_system_organization: @maxmind_results.traits.autonomous_system_organization,
        city_name: @maxmind_results.city.name,
        location_metro_code: @maxmind_results.location.metro_code,
    ...
    def create_or_update_data_center
      ...
      if @data_center_exists
        message = <<-EOS
          ...
          overridden by actual data from maxmind.
        EOS
        ...
        message = <<-EOS
          ...
          created (##{@data_center.id}) from maxmind data.
        EOS
    ...
    def assign_validator_to_new_data_center
      # If data center from maxmind is the same as already assigned - do nothing, it's ok.
      if @data_center.id == @validator&.data_center&.id
        message = <<-EOS
          Data center from maxmind is the same that is assigned to validator, no need to re-assign.
        EOS
