...
leaders_data = Blockchain::MainnetBlock.joins("JOIN blockchain_mainnet_slots ON blockchain_mainnet_slots.slot_number = blockchain_mainnet_blocks.slot_number").select("blockchain_mainnet_slots.epoch AS epoch, blockchain_mainnet_slots.leader AS leader, blockchain_mainnet_blocks.slot_number AS slot_number").where("blockchain_mainnet_slots.epoch = ?", 684).group_by(&:leader)
