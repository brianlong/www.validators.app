...
class ClusterStatsControllerTest < ActionDispatch::IntegrationTest
  ...
  def assert_data_for(network, data)
    ...
    skipped_slot_stats = { min: 0.25, max: 0.25, median: 0.25, average: 0.25 }


    assert_equal stats[:batch_uuid], Batch.last_scored(network).uuid
    assert_equal top_staked_validators, stats[:top_staked_validators], 'Top Staked Validators'
    assert_equal top_skipped_vote_validators, stats[:top_skipped_vote_validators], 'Top Skipped Vote'
    assert_equal top_root_distance_validators, stats[:top_root_distance_validators], 'Top Root Distance'
    assert_equal top_vote_distance_averages_validators, stats[:top_vote_distance_validators], 'Top Vote Distance'
    assert_equal top_skipped_slot_percent, stats[:top_skipped_slot_validators], 'Top Skipped Slot'
