...
class ClusterStatsControllerTest < ActionDispatch::IntegrationTest
  ...
  def assert_data_for(network, data)
    ...
    assert_equal top_skipped_vote_validators, stats[:top_skipped_vote_validators].map(&:first), 'Top Skipped Vote'
    ...
    assert_equal top_skipped_slot_percent, stats[:top_skipped_slot_validators].map(&:first), 'Top Skipped Slot'
